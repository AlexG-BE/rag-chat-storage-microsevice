...
from factory import BUILD_STRATEGY, Factory, FactoryError, base, errors
...
class AsyncStepBuilder(StepBuilder):
    async def build(
        ...
        for declaration_name in post.sorted():
            ...
            if asyncio.iscoroutinefunction(declaration.declaration.evaluate_post):
                postgen_results[declaration_name] = await declaration.declaration.evaluate_post(
                    instance=instance,
                    step=step,
...
# https://github.com/kuzxnia/async_factory_boy/pull/8
class AsyncSQLAlchemyFactory(Factory[base.T]):
    ...
    @classmethod
    async def build(cls, **kwargs) -> base.T:
        return await cls._generate(strategy=BUILD_STRATEGY, params=kwargs)

    @classmethod
    async def build_batch(cls, size, **kwargs) -> list[base.T]:
        return [await cls.build(**kwargs) for _ in range(size)]

    @classmethod
    async def create(cls, **kwargs) -> base.T:
    ...
    def _create(cls, model_class, *args, **kwargs):
        session = cls._meta.sqlalchemy_session  # noqa: F841
    ...
    @classmethod
    async def create_batch(cls, size, **kwargs) -> list[base.T]:
